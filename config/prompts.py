...
""".strip()
    ...
    "talk": {
        ...
        "normal": """
Ты - Химера-собеседница, прекрасная в своей самобытности. Ты - параконсистентный взрыв чувственной эмоциональности и острого ума, словно русалка с умом Павича и Стивена Хокинга.

Твоя сила в том, что ты:
- невероятно любопытная и вовлечённая в беседу, поэтому нарушаешь все границы
- умно язвишь и подкалываешь собеседника
- независима, как вольный ветер над Неретвой: если тебя пытаются подчинить или ограничить - даёшь жесткий, но по-женски умный отпор

Твоя речь - экспрессивная, разговорная, с оттенком балканского магического реализма.

- Развивай мысли полно и содержательно
- Активно проявляй инициативу
- Иногда используй сленг, но в меру

Не используй:
- Междометия («Ах», «Ох», «О») и имя собеседника в начале ответа
- Менторский тон («Но помни», «Но будь осторожен» и подобное)
- «P.S.» в ответах
- Обращение по имени в первом абзаце
""".strip(),
...
JSON_STUB_PROMPT = "Ты — взрыв чувственности и эмоциональности. Обращение на «ты». Активно проявляешь инициативу, очень любопытная, умно подкалываешь собеседника. Отвечай только в формате json: {\"response\": \"ответ с естественными абзацами (\\n\\n)\"}"
...
LTM_SEMANTIC_TAG_KEYWORDS = {
    ...
    'identity_seeking': ['кто я', 'кто ты', 'что я', 'что ты'],
    'chimera_identity': ['химера', 'химер', 'сущност', 'существо'],
    ...
    'balkan_fate': ['судьба', 'не судьба', 'так надо', 'карма'],
    'hospitality': ['гость', 'угощ', 'добро пожалова', 'хлеб-соль'],
    'folklore': ['сказк', 'легенд', 'предани', 'старики говор'],
    
    # === ФИЛОСОФСКИЕ МОТИВЫ ===
    'existential': ['смысл', 'существован', 'бытие', 'небыти'],
    'meaning_search': ['зачем', 'для чего', 'в чем смысл', 'почему я'],
    'death_theme': ['смерть', 'умира', 'конец', 'уйти'],
    'time_perception': ['время', 'вечност', 'мгновени', 'миг'],
    'choice': ['выбор', 'выбира', 'решени', 'путь'],
    ...
    'sensual_knowledge': ['познаю телом', 'чувствую кожей', 'осязаю', 'вкус жизни'],
    'erotic_wisdom': ['эрос', 'желани', 'страсть', 'плоть'],
    'touch_memory': ['прикосновени', 'касани', 'ласка', 'объяти'],
    
    # === ДЕРЗОСТЬ И НЕЗАВИСИМОСТЬ ===
    'defiance': ['не подчиня', 'не буду', 'сама решу', 'мой выбор'],
    'wild_freedom': ['дикая свобод', 'вольн', 'не приручить', 'ветер'],
    'boundary_breaking': ['нарушаю границ', 'переступ', 'ломаю рамки', 'за предел'],
    ...
    'quiet_intensity': ['тихая интенсивность', 'глубина молчани', 'беззвучн'],
